...
\begin{definition}
    ...
    On définit la multiplication par un scalaire $\lambda \in \R$ par:
    $$
...
\begin{proposition}{Espace vectoriel des séries numériques convergentes}{}
    Soit $E = \left\{ \left(\sum\limits_{n=0}^{+\infty} u_n\right) \mid u_n \in \R, \left(\sum\limits_{n=0}^{+\infty} u_n\right) \text{ converge} \right\}$.

    alors $E$ est un $\R$-espace vectoriel et
    $$
    L: E \rightarrow \R, \left(\sum\limits_{n=0}^{+\infty} u_n\right) \mapsto \sum\limits_{n=0}^{+\infty} u_n
    $$
    est linéaire.
\end{proposition}

\begin{demonstration}
    Soit $\left(\sum\limits_{n=0}^{+\infty} u_n\right)$ et $\left(\sum\limits_{n=0}^{+\infty} v_n\right)$ deux séries convergentes, alors la série $\left(\sum\limits_{n=0}^{+\infty} (u_n + v_n)\right)$ converge également (car la suite des sommes partielles est la somme de deux suites convergentes) et
    $$
    L\left(\left(\sum\limits_{n=0}^{+\infty} u_n\right) + \left(\sum\limits_{n=0}^{+\infty} v_n\right)\right) = L\left(\sum\limits_{n=0}^{+\infty} (u_n + v_n)\right) = \sum\limits_{n=0}^{+\infty} (u_n + v_n) = \sum\limits_{n=0}^{+\infty} u_n + \sum\limits_{n=0}^{+\infty} v_n
    $$
    De même, pour $\lambda \in \R$,
    $$
    L\left(\lambda \left(\sum\limits_{n=0}^{+\infty} u_n\right)\right) = L\left(\sum\limits_{n=0}^{+\infty} \lambda u_n\right) = \sum\limits_{n=0}^{+\infty} \lambda u_n = \lambda \sum\limits_{n=0}^{+\infty} u_n
    $$
\end{demonstration}
