...
\begin{definition}
    ...
    $(u_n)$ est une \textbf{suite de Cauchy} si et seulement si
    $$
    \forall \varepsilon > 0,\; \exists N \in \N,\; \forall p, q \geq N, \quad |u_p - u_q| < \varepsilon
    $$
\end{definition}
...
\begin{demonstration}
    \begin{description}
        \item[sens direct.]
            Si $\xn{u}$ converge vers $l \in \R$, on a
            $$
            \forall \varepsilon > 0,\; \exists N \in \N,\; \forall n \geq N,\quad |u_n - l| < \dfrac{\varepsilon}{2}
            $$
            Donc pour $p, q \geq N$:
            $$
            |u_p - u_q| = |u_p - l + l - u_q| \leq |u_p - l| + |u_q - l| < \dfrac{\varepsilon}{2} + \dfrac{\varepsilon}{2} = \varepsilon
            $$
...
\section{Série numériques et complexes}

Dans cette section, on considèra que ce qui fonctionne pour $\R$ fonctionne aussi pour $\C$.

\subsection{Définitions et premières propriétés}

\begin{definition}
    On appelle série numérique (ou complexe) la donnée de deux suite réelles
    $(u_n, S_n)_{n \in \N}$ telles que $S_n = \sum\limits_{k=0}^{n}u_k$.\n
    On dit que la série converge si il existe $S \in \R$ (ou $\mathbb{C}$) tel que $S_n \underset{n \to +\infty}{\rightarrow} S$.\n
    ...
    \begin{itemize}[label=--]
        ...
        \item
        $u_n$ est appelé \textbf{terme général}.
        \item
...
\begin{exemple}
    ...
    On a $S_n = \sum\limits_{k=0}^{n} k = \dfrac{n(n+1)}{2} \rightarrow +\infty$ donc la série diverge.
\end{exemple}
...
\begin{demonstration}
    En dérivant la somme partielle $S_n := \sum\limits_{k=0}^{n} x^k = \dfrac{1 - x^{n+1}}{1 - x}$, on trouve:
    $$
    S_n'(x) = \sum\limits_{k=1}^{n}kx^{k-1}
    $$
    mais aussi
    \begin{align*}
        S_n'(x) &= \dfrac{-(n + 1)x^n(1 - x) + (1 - x^{n+1})}{(1 - x)^2} \\
        &= \dfrac{1}{(1 - x)^2} - \dfrac{(n+1)x^n(1 - x) + x^{n + 1}}{(1 - x)^2} \\
        &= \dfrac{1}{(1 - x)^2} - \dfrac{(n+1)x^n}{1 - x} - \dfrac{x^{n + 1}}{(1 - x)^2}
    \end{align*}

    Ainsi
    \begin{align*}
        \sum\limits_{k=1}^{N}kx^{k-1} &= \dfrac{1}{(1 - x)^2} - \dfrac{(n+1)x^n}{1 - x} - \dfrac{x^{n + 1}}{(1 - x)^2}\\
        \implies \sum\limits_{n=1}^{+\infty}nx^{n} &= \dfrac{x}{(1 - x)^2} - \dfrac{(n+1)x^{n+1}}{1 - x} - \dfrac{x^{n + 2}}{(1 - x)^2}
    \end{align*}

    \begin{description}
        \item[Si $|x| < 1$:]
            $\dfrac{(n+1)x^{n+1}}{1 - x} \underset{n \to +\infty}{\longrightarrow} 0$ et $\dfrac{x^{n + 2}}{(1 - x)^2} \underset{n \to +\infty}{\longrightarrow} 0$ donc la série converge et
            $$
            \sum\limits_{n=1}^{+\infty}nx^{n} = \dfrac{x}{(1 - x)^2}
            $$

        \item[Si $|x| > 1$:]
            $\dfrac{(n+1)x^{n+1}{1 - x}} \underset{n \to +\infty}{\longrightarrow} +\infty$ et $\dfrac{x^{n + 2}}{(1 - x)^2} \underset{n \to +\infty}{\longrightarrow} +\infty$ donc la série diverge.
        \item[Si $|x| = 1$:]
            $S_n'(x) = \sum\limits_{k=1}^{n}k = \dfrac{n(n+1)}{2} \underset{n \to +\infty}{\longrightarrow} +\infty$ donc la série diverge.
    \end{description}
\end{demonstration}
