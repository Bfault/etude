...
\begin{demonstration}
    On montre la proposition pour $g_\varphi$, la démonstration étant symétrique pour $d_\varphi$.

    Il est calir que $g_\varphi$ associe à tout $u \in E$ une application linéaire de $F$ dans $\K$. De plus
    $$
    g_\varphi(u + \lambda v) = \varphi(u + \lambda v, \cdot)
    $$
    et donc
    $$
    \forall w \in F, \varphi(u + \lambda v, w) = \varphi(u, w) + \lambda \varphi(v, w)
    $$
    et donc
    $$
    g_\varphi(u + \lambda v) = g_\varphi(u) + \lambda g_\varphi(v)
    $$
    Donc $g_\varphi$ est linéaire.
\end{demonstration}
...
\begin{demonstration}
    Si on définit la somme et le produit externe par un scalaire comme suit
    ($\varphi_1$ et $\varphi_2$ sont des formes bilinéaires, $\alpha \in \R$):
    \begin{align*}
        (\varphi_1 + \varphi_2)(x, y) = \varphi_1(x, y) + \varphi_2(x, y) \\
        (\alpha \varphi_1)(x, y) = \alpha \varphi_1(x, y)
    \end{align*}
    Il est aisé de vérifier que ces deux opérations font de $\mathcal{L}_2(E)$ un $\K$-espace vectoriel.
\end{demonstration}
